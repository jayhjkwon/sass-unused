...
function gatherUsedVars(rootNode, path) {
  ...
  visitSassExpressions(rootNode, (expr) => {
    ...
    if (idents) {
      idents.forEach((id) => used.add({ id, path }));
    }
  });
  return used;
}
...
function findUnused(srcFiles, resolver) {
  ...
  srcFiles.forEach((path) => {
    ...
    gatherUsedVars(rootNode, path).forEach((ident) => {
      usedVars.add(ident.id);
    });

    declaredMixins.push(...gatherDeclaredMixins(rootNode, path));
    gatherUsedMixins(rootNode, path).forEach((ident) => {
      usedMixins.add(ident.id);
    });

    declaredFunctions.push(...gatherDeclaredFunctions(rootNode, path));
    gatherUsedFunctions(rootNode, path).forEach((ident) =>
      usedFunctions.add(ident.id)
    );
  });

  const unusedVars = declaredVars.filter((ident) => !usedVars.has(ident.id));
  const usedVarsWithPath = declaredVars.filter((ident) =>
    usedVars.has(ident.id)
  );
  const unusedMixins = declaredMixins.filter(
    (ident) => !usedMixins.has(ident.id)
  );
  const usedMixinsWithPath = declaredMixins.filter((ident) =>
    usedMixins.has(ident.id)
  );
  const unusedFunctions = declaredFunctions.filter(
    (ident) => !usedFunctions.has(ident.id)
  );
  ...
  return {
    ...
    usedVars: _.uniqBy(usedVarsWithPath, "id"),
    usedMixins: _.unionBy(usedMixinsWithPath, "id"),
  };
}
